...
public class GraphExam {
   public static void main(String[] args) {
      ...
      // See the code below X to see more info.


      // QUESTION: VIII
      // Assumes index is the query
      // Single case.
      // Works with any sourceVertex; returns -1 if does not exist along with a error
      // message.
      ...
      //   with the isEdge method, we only need to go over it a single
      //   time.

      System.out.println("Graph has loop? " + Graph.hasLoops(myNet));
      myNet.addEdge(0, 0); // Now that we have a loop, it will return true.
      System.out.println("Graph has loop? " + Graph.hasLoops(myNet));
